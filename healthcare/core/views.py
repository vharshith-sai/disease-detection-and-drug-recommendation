...
# Global preload of machine learning models to prevent memory exhaustion
MODEL_DIR = os.path.join(settings.BASE_DIR, 'model')
try:
    disease_model = joblib.load(os.path.join(MODEL_DIR, 'decision_tree.pkl'))
    drug_model = joblib.load(os.path.join(MODEL_DIR, 'medical_nb.pkl'))
except Exception as e:
    print(f"Warning: Could not load machine learning models during initialization. {e}")
    disease_model = None
    drug_model = None
...
def MakePredict(request):
    ...
    clf = disease_model
    if (clf is None):
        return JsonResponse({'status': 'Model not loaded'})
